...
void parse()
{
  for (string call; getline(cin, call);)
  {
    ...
    while (getline(cin, call) && !call.empty())           //read lines from call graph
    {
      ...
      map<string, set<string> >::iterator scope;
      scope = all_scopes.find(name);
      if (ipa && scope != all_scopes.end())
      {
        //expand
        set<string>* expand = &all_scopes[name];
        set<string>::iterator iter;
        for (iter = expand->begin(); iter != expand->end(); iter++)
        {
          //insert functions from scope into this scope
          functions->insert(*iter);
        }
      }
      else
      {
  ...
  for (iter = all_scopes.begin(); iter != all_scopes.end(); iter++) //iterate through all scopes
  {
    ...
    for (set<string>::iterator scope_iter = functions->begin(); scope_iter != functions->end(); scope_iter++)
    {
      ...
      for (set<string>::iterator conn_iter = connections->begin(); conn_iter != connections->end(); conn_iter++)
      {
        ...
        {
          cout << "bug: " << function << " in " << scope << ", pair: (" << pair.first << ", " << pair.second << "), support: " << edge->support << ", confidence: " << fixed << setprecision(2) << conf;
          cout << '%' << endl;
        }
      }
    }
